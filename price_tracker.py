"""
DigiKey Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from DigiKey.

Docs  : https://docs.bright.cn/scraping-automation/web-scraper-api/overview
Info  : https://bright.cn/products/insights/price-tracker/digikey
"""
...
# Search for "DigiKey" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Navigate to: https://bright.cn/cp/scrapers\n"
        "2. Search for 'DigiKey' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of DigiKey product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover DigiKey products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a DigiKey category page.

    Example:
        results = discover_by_category("https://digikey.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.digikey.com/product/sample-item-123456",
        # Add more DigiKey product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "digikey_prices.json"
    ...
    # cat_results = discover_by_category("https://digikey.com/category/sample", limit=50)
